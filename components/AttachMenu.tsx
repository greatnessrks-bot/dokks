...
import { Plus, Camera, Image as ImageIcon, X, AlertCircle } from "lucide-react";
...
interface Props {
  ...
  onUploadProgress?: (active: boolean, progress: number) => void;
  disabled?: boolean;
}

export default function AttachMenu({ onParsed, onUploadProgress, disabled }: Props) {
  const { t } = useSettings();
  ...
  async function handleImageSelected(file: File) {
    setError(null);
    setProcessing(true);
    onUploadProgress?.(true, 0);
    try {
      const parsed = await parseImageFile(file, (pct) => {
        onUploadProgress?.(true, pct);
      });
      onParsed(parsed);
    } catch (err) {
  ...
  return (
    <>
      ...
      {error && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[60] w-[calc(100%-2rem)] max-w-md px-4 animate-[slideDownIn_0.25s_ease-out]">
          <div className="flex items-start gap-2.5 rounded-lg border border-accent-amber/40 bg-surface shadow-lg px-4 py-3">
            <AlertCircle className="w-4 h-4 text-accent-amber shrink-0 mt-0.5" />
            <p className="flex-1 text-sm text-foreground leading-snug">{error}</p>
            <button
              type="button"
